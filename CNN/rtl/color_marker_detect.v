...
localparam [1:0] DIV_RED_X  = 2'd0;
localparam [1:0] DIV_RED_Y  = 2'd1;
localparam [1:0] DIV_BLUE_X = 2'd2;
localparam [1:0] DIV_BLUE_Y = 2'd3;
...
assign red_hit =
    ...
    (tap_g <= red_cfg_reg[15:8]) &&
    (tap_b <= red_cfg_reg[23:16]);
assign blue_hit =
    (tap_r <= blue_cfg_reg[7:0]) &&
    (tap_g <= blue_cfg_reg[15:8]) &&
...
assign scheduled_denominator =
    ((div_target == DIV_RED_X) || (div_target == DIV_RED_Y)) ?
        final_red_count : final_blue_count;
            ...
            case (state)
                ...
                S_SCHEDULE: begin
                    if (target_has_nonzero_count) begin
                        ...
                        state <= S_DIVIDE;
                    end else if (div_target == DIV_BLUE_Y) begin
                        state <= S_COMPLETE;
                    end else begin
                ...
                S_DIVIDE: begin
                    if (div_cycle < 6'd28) begin
                        ...
                        if (div_target == DIV_BLUE_Y) begin
                            state <= S_COMPLETE;
                        end else begin
                            div_target <= div_target + 1'b1;
                            state <= S_SCHEDULE;
