...
보드에서 처음 켤 때 이 순서로 확인합니다. 호스트 테스트와 ARM 링크까지는 확인했지만 보드 실행은 아직 확인하지 않았습니다.

1. **빌드**: Vitis에서 프로젝트를 새로고침(F5)하고 Debug 설정에 `ROBOT_TRACE`를 추가한 뒤 Clean → Build 합니다.
   에러가 없고 빌드 로그에 `[TRACE] ROBOT_TRACE enabled...` 안내가 한 줄 나오면 됩니다.
2. **배너**: 시리얼 터미널을 921600으로 열고 Debug As로 실행합니다. `[platform] ready (tick 20 ms, uart 921600 baud, trace on)` 배너가 나오고,
   이어서 `#A1,...` 같은 컬럼 줄, `EV,0,BOOT,921600`, 그리고 1초마다 `SM` 줄이 나오면 baud와 링크가 살아 있는 것입니다(pose를 보내지 않아도 여기까지 나옵니다).
   확인한 뒤 터미널을 닫습니다(같은 COM 포트입니다).
3. **1분 실행**: [로그 받기](#로그-받기)의 명령으로 1분쯤 돌리고 아래를 확인합니다.
   ```powershell
   (sls '^A1,' run.log).Count; (sls '^TK,' run.log).Count   # 줄 수
   sls '^SM,' run.log | select -Last 3                        # 최근 SM
   sls '^EV,' run.log                                         # 이벤트
   ```
   - `A1`, `P3`, `A2`가 초당 약 20줄, `TK`가 초당 약 50줄입니다.
   - SM에서 `ovr=0`, `crc=fmt=rng=0`, `drop=0`이고 `hi`가 작습니다(8192면 링버퍼 가득).
   - `A1`의 `fid`와 `TK`의 `tick`이 1씩 끊김 없이 늘어납니다.
   - 로봇 움직임이 trace를 끈 빌드와 같습니다.
4. **실행 시간**: `dur_us`를 봅니다. Debug(`-O0`)에서 `A1`은 대략 500~1000 µs로 추정합니다(추정이므로 크게 벗어나면 기록해 두세요).
5. **거부 사유**: 팔을 쭉 뻗게 해서 `A2`의 `st=R`과 `fg`가 나오는지 봅니다(`0x10`이면 도달 한계).
...
| 증상 | 원인과 조치 |
|---|---|
| 글자가 깨지거나 `crc`가 오르거나 로그가 없음 | UART 속도 문제입니다. PC를 `--baud 921600`으로 열었는지 확인하세요. 계속 안 되면 `include/integration/trace.h`의 `ROBOT_TRACE_UART_BAUD`를 `460800U`로 바꿔 다시 빌드하고 PC도 `--baud 460800`으로 엽니다(921600의 실제 통신은 아직 보드에서 확인하지 않았습니다). 빌드 로그의 안내 문구는 921600으로 고정돼 있으니 실제 값은 배너와 `EV,BOOT`로 확인하세요 |
| 로그는 없는데 로봇은 정상 | `ROBOT_TRACE`가 Debug 설정에 없거나 옛 ELF입니다. 배너에 `trace on`이 있는지 확인하세요 |
| SM의 `ovr`이 0이 아님 | main 루프가 20 ms 안에 못 돌았습니다. trace를 끈 빌드와 비교하고 `A1`/`A2`/`TK`의 `dur_us`를 확인하세요 |
| SM의 `drop`이 0이 아님 | 링버퍼가 찼습니다(`hi`가 8192에 가까움). 링크 속도에 비해 로그가 많은 것입니다 |
| `fid`나 `tick`이 끊김 | 보드의 `drop`이 0인데 끊기면 PC 수신 버퍼 문제입니다(`--hz`가 낮음). `drop`이 있으면 링버퍼 문제입니다 |
| 서보 PWM이 안 나옴 | 서보 드라이버가 mock으로 빌드됐는지 확인하세요([주의](#주의) 6번) |
